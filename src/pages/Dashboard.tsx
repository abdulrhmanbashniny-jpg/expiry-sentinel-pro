...
import { AlertTriangle, Calendar, CheckCircle, Clock, FileText, Plus, XCircle } from 'lucide-react';
...
const Dashboard: React.FC = () => {
  ...
  const { data: expiringData, isLoading } = useExpiringItems();
  const { data: notifications } = useRecentNotifications();

  const stats = [
    { label: 'منتهية', count: expiringData?.expired.length ?? 0, icon: XCircle, color: 'text-destructive', bg: 'bg-destructive/10' },
    { label: 'خلال 7 أيام', count: expiringData?.expiring7.length ?? 0, icon: AlertTriangle, color: 'text-warning', bg: 'bg-warning/10' },
    { label: 'خلال 14 يوم', count: expiringData?.expiring14.length ?? 0, icon: Clock, color: 'text-primary', bg: 'bg-primary/10' },
    { label: 'خلال 30 يوم', count: expiringData?.expiring30.length ?? 0, icon: Calendar, color: 'text-success', bg: 'bg-success/10' },
  ];
                    ...
                    >
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {item.category?.name} • {format(new Date(item.expiry_date), 'dd/MM/yyyy')}
                        </p>
                      </div>
            ...
            {!notifications || notifications.length === 0 ? (
              ...
              <div className="space-y-3">
                {notifications.slice(0, 5).map((notif) => (
                  <div key={notif.id} className="flex items-center justify-between rounded-lg border p-3">
                    <div>
                      <p className="font-medium">{notif.item?.title}</p>
                      <p className="text-sm text-muted-foreground">
                        {notif.recipient?.name} • {formatDistanceToNow(new Date(notif.created_at), { locale: ar, addSuffix: true })}
                      </p>
                    </div>
                    <Badge variant={notif.status === 'sent' ? 'default' : notif.status === 'failed' ? 'destructive' : 'secondary'}>
                      {notif.status === 'sent' ? 'تم الإرسال' : notif.status === 'failed' ? 'فشل' : 'قيد الانتظار'}
                    </Badge>
                  </div>
                ))}
              </div>
